...
Blackwell GPU with Python 3.12 and Rezo's native `sm_120` TensorFlow wheel.

```bash
python3.12 -m venv venv-blackwell
source venv-blackwell/bin/activate
pip install --upgrade pip

export UV_KEYRING_PROVIDER=subprocess
export GCP_TOKEN="$(gcloud auth print-access-token)"
export REZO_PYPI="https://oauth2accesstoken:${GCP_TOKEN}@us-central1-python.pkg.dev/"
REZO_PYPI="${REZO_PYPI}gke-test-421317/rezo-pypi/simple/"
PIP_CONSTRAINT= pip install \
  --extra-index-url "${REZO_PYPI}" \
  "tensorflow==2.22.0+rezo.blackwell"
pip install --no-deps tf-keras-nightly
pip install docopt rdkit scipy scikit-learn pandas joblib planarity
export TF_USE_LEGACY_KERAS=1
```

Clear `PIP_CONSTRAINT` for the TensorFlow install if the base image pins older
dependency versions.
